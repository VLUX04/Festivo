...
import { getUserIdByUsername } from './user.service.js';

export async function initiateChat(currentUsername: string, friendUsername: string) {
	const currentUser = await getUserIdByUsername(currentUsername);
	const friendUser = await getUserIdByUsername(friendUsername);
...
export async function sendChatMessage(currentUsername: string, chatId: string, content: string) {
	const sender = await getUserIdByUsername(currentUsername);
